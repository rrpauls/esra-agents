...
function settings(api: any) {
  const config = (api.pluginConfig ?? {}) as JsonObject;
...
function agentId(api: any, ctx: JsonObject): string {
...
function staticFindings(event: any): Array<{ ruleId: string; severity: "critical"; message: string; file?: string }> {
  const findings: Array<{ ruleId: string; severity: "critical"; message: string; file?: string }> = [];
...
export default {
  ...
  register(api: any) {
    api.on("model_call_ended", async (event: any, ctx: JsonObject) => {
    ...
    api.on("after_tool_call", async (event: any, ctx: JsonObject) => {
      await ingest(api, ctx, {
    ...
    api.on("agent_end", async (event: any, ctx: JsonObject) => {
      await ingest(api, ctx, {
    ...
    api.on("skill_changed", async (event: any, ctx: JsonObject) => {
      await ingest(api, ctx, {
