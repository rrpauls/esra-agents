...
OpenClaw runtime evidence observed in an isolated `OPENCLAW_STATE_DIR`: exact `v0.2.2` Git installation, compiled JavaScript loading, seven registered native hooks, clean runtime inspection, a scanner-blocked protected candidate, a scanner-clean candidate, exact authenticated apply, one-time authorization rejection on reuse, canary rollback to an absent snapshot, and 30-day terminal candidate cleanup.
